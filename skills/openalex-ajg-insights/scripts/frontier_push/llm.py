...
def build_interest_profile_prompt(intent: str, requested_directionality: str | None = None) -> str:
    lines = [
            ...
            "- Provide at least two precise established exact aliases per group so round 1 uses OR within every concept group.",
...
def build_profile_audit_prompt(intent: str, profile: InterestProfile) -> str:
    ...
    return "\n".join(
        [
            ...
            "- Return needs_revision when any descriptive concept group contributes fewer than two exact aliases to round 1, unless the user explicitly requested exact-only single-anchor retrieval.",
            "- Do not return pass when a shorter established lexical anchor would preserve the concept but the profile uses only longer variants.",
